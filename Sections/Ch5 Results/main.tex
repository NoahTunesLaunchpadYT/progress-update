...
AGX, and the NVIDIA Xavier NX. Worst case, the desktop workstation 
performed at 150 frames per second, the Xavier AGX performed at 25 frames per second, and the Xavier 
NX performed at 20 frames per second.  
...
high noise. On each trial, the complete pipieline is run up until 
...
a frame rate of above 300 frames per seconds was achieved for lower integration segment counts 
...
frames. Utilising two or four processing buffers allowed the pipeline to maintain 150+ framerate 
even when the integration stage didn't compress the data at all (256 segments). Another finding 
...
With no pipelining, framerate is the reciprocal of latency, and hence only the latency of the 
pipeline is reported for these devices. On the AGX, a latency of less than 40 ms is achieved 
worst case, and on the NX, that number is less than 50 ms (see Figures 
\ref{fig:latency-breakdown-agx} and \ref{fig:latency-breakdown-nx}).

% REDO THIS PART BELOW
