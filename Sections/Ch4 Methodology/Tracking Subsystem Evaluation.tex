...
\subsection{CFAR}
For finding the CFAR coefficient $\alpha$ (defined in \ref{eqn:threshold}) 
that gives a desired false 
alarm rate $P_{fa}^{\star}$, a technique novel to field of UAV detect and avoid is applied. 
While works like Spencer et al. \cite{compact_spencer_2015} apply \gls{cfar} after angle sensing, we apply
\gls{cfar} before angle sensing. Additionally, unlike the works that find detections before angle 
sensing \cite{Wang2016, Moses2014}, which threshold each channel independently, we apply \gls{cfar} across
across every possible quadruplet of four receivers in our array, improving SNR power by a factor of four 
(groupings shown in Figure 
\ref{fig:overlapping_quadruplets}). This \gls{cfar} strategy obtains the pre-angle-sensing compute savings 
Wang et al.
seeks while recovering the compute that Spencer wastes by performing angle sensing on samples that will
get masked out sacrifices. 
As identified in the Background \ref{sec:background-radar-processing},
the power sum across multiple receivers is a chi-squared distributions, for which the relationship between 
false alarm rate, the number of training 
cells, and the threshold multiplier $\alpha$ is known \cite{Richards2005FundamentalsRadarSP}. 

The relationship cannot be rearranged into a closed-form expression. 
...
\begin{figure}
    \centering
    \includesvg[width=0.9\linewidth]{pfa_vs_alpha.svg}
    \caption[False alarm rate as a function of the CFAR coefficient $\alpha$.]{False alarm rate as a function 
    of the CFAR coefficient $\alpha$ for different numbers of training cells $N$. The relationship is strictly 
    decreasing, allowing for bisection to find the value of $\alpha$ that gives the desired false alarm rate 
    \(P_{fa}^{\star}\).}
    \label{fig:pfa_vs_alpha}
\end{figure}

Since $P_{fa}(\alpha;N)$ is strictly decreasing in $\alpha$ (see Figure \ref{fig:pfa_vs_alpha}), bisection can be used 
...
conducted for each CFAR implementation on datasets DS2, and DS3. The performance (throughput, latency, 
...
algorithms, including CA-CFAR, SO-CFAR, and GO-CFAR. The effect of these algorithms on detection
...
efficiency loss from processing so many zero samples motivates a stage that transforms 
the detection-sparse representation into a detection-dense one. Additionally, the 
widening of the main lobe of each of the detections due to the windowing stage creates 
multiple range bins with detections from the same target. The algorithm that addresses 
both of these problems, clustering, iterates through the range profile and creates a 
cluster for each contiguous set of detection samples. Then, the complex signals from 
each of the samples are summed together (see Figure \ref{fig:clustering-sparse-dense}). 
...
coherent. Clustering is the stage at which the main-lobe width cost of windowing is paid, 
and its behaviour is therefore coupled to the choice of window and the parameters of 
the prior CFAR stage.
...
sweeps over SNR and number of targets, and measure angular bias, and \gls{rms} distance error. 
...
for each time step, iterating through each of the lists and sorting them into one 
ordered list; this is a merge sort of n sorted lists.  
The algorithm then iterates through the ordered list and eliminates detections that are close 
in range to a stronger detection. This stage was not implemented on the GPU, as it is the precursor to
a CPU stage (tracking) and the latency of this stage is expected to be small, since most samples
have been discarded and combined. This is an assumption that will be verified through the ablation study. 
...
The multi-target tracking stage will be implemented entirely on the CPU due to its sequential nature. Experiments will
be conducted across variations in SNR, and number of targets, in simulated data (DS2) and pseudo-real data (D4). 
