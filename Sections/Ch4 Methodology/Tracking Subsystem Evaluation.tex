...
start and ends of the left and right window (see 
...
\begin{figure}[htbp]
    ...
    \label{fig:gpu-cfar}
    \caption[Parallel GPU CFAR implementation.]{CFAR implementation on the GPU. Threads (each depicted as a unique colours) are each
...
The literature gap regarding algorithmic selection is address through an exploration of different CFAR
...
the difference in performance of the algorithms on a synthetic signal with 5 peaks, 

\begin{figure}
    \centering
    \includesvg[width=0.81\linewidth]{Images/cfar_stacked_matched_colours}
...
detection samples. Then, the complex signal from each of the samples are summed together. 
...
\begin{figure}[htbp]
    ...
    \begin{minipage}{0.5\textwidth}
        ...
        still receive significant power from the 1st sidelobes lining up with the target. }
        \label{fig:sidelobes}
    \end{minipage}
...
The blanking stage will be implemented on both the CPU and GPU. After clustering and angle sensing, 
...
a stronger detection. A GPU implementation would involve creating a thread per detection per quadruplet, 
...
The multi-target tracking stage with be implemented entirely on the CPU due to its sequential nature. 
Amplitude based association will be compared to non-amplitude based association for their impact on
track quality. The multi-target tracker has over 20 parameters that can be tuned, and a sweep of 
these parameters will be conducted to understand their impact on track quality. Experiments will
be conducted in a variation of SNR, number of targets, and target ranges, in simulated data. Real 
