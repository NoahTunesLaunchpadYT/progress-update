...
propagation, data corruption, and non-gaussian noise. 

An interactive variation of this dataset is 
...
While every component of the software can be tested with this first dataset, stages such as windowing, CFAR, angle sensing, and tracking benefit from more 
realistic data. The choice of window heavily relies
...
clutter, and other phenomena that are not captured in the synthetic datasets. 

The fourth dataset combines the realism of the third dataset with the controllability of the second. 
It is a pseudo-real dataset 
that incorporates the clutter and noise characteristics from a recording of an airborne radar pointing at an 
empty scene with synthetic targets from the physics simulation. 
Table \ref{tab:datasets} summarises the datasets and their characteristics.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{|c|p{3cm}|p{5cm}|p{5cm}|}
        ...
        non-Gaussian noise. \\
        \hline
        DS3 & Validation under air-to-air conditions & Air-to-air radar data recorded from the 
        AWR2243 radar system. Contains all expected non-idealities. & 
        Contains timing uncertainties and is in limited supply. \\
        \hline
        DS4 & Validation under air-to-air conditions & Air-to-air background noise with simulated targets added. &
        Does not model non-idealities of target signals such as multipath propagation. \\
        \hline
    \end{tabular}
    \label{tab:datasets}
\end{table}