...
which critically evaluates over 20 papers across four domains: UAV DAA radar sensing,
...
Zaknoun et al.\ \cite{zaknoun2024_realtime_gpu_accelerated_radar_processing} 
and the GPU benchmarking gap in Newmeyer et al.\ \cite{7577322}), and design decisions
...
trade-off under SWaP constraints. This aligns with the unit's theme of optimal 
control and estimation, and Week 09's content (System uncertainty).
...
incoherent noise. The Kalman filter's prediction and update equations use \textbf{covariance
matrices} to propagate measurement and process uncertainty, and the Mahalanobis
distance is a covariance-normalised scalar used for gating. These elements together
...
window function is characterised through its frequency-domain response. Windowing
can be modelled as a \textbf{convolution} in the frequency domain, and this understanding
is used to link the window's shape to sidelobe attenuation and mainlobe width.  
The effect of low SNR on signal model validity is studied
