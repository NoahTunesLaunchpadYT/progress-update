...
where k is one less than the number of receiver channels \cite{Richards2010}. In this case, \(k=3\). Figure \ref{fig:cfar_detection} illustrates how CFAR generates detections by comparing a signal to a threshold determined by the power of the surrounding samples.

\begin{figure}
    \incfig[1]{figures/cfar_detection}
    \caption{CFAR detection process. The threshold is calculated based on the power of surrounding samples, and the signal is compared to this threshold to generate detections.}
